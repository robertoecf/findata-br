...
bcb_app = typer.Typer(help="Banco Central do Brasil data")
...
b3_app = typer.Typer(help="B3 stock exchange data")
...
def b3_quote(
    ...
    rprint(f"  [green]Preco:[/green] R$ {q.preco:.2f}" if q.preco else "  Preco: N/A")
    rprint(f"  [green]Variacao:[/green] {q.variacao_dia:+.2f}%" if q.variacao_dia else "")
    rprint(f"  Abertura: {q.abertura:.2f}" if q.abertura else "")
    rprint(f"  Volume: {q.volume:,}" if q.volume else "")
    rprint(f"  Setor: {q.setor}" if q.setor else "")
...
tesouro_app = typer.Typer(help="Tesouro Direto treasury bonds")
...
ibge_app = typer.Typer(help="IBGE economic indicators")
...
def serve(
    ...
    rprint(f"[green]Starting findata-br on {host}:{port}[/green]")
    rprint(f"  REST API: http://{host}:{port}/docs")
